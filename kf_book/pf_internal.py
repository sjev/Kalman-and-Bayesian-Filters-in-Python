...
def plot_monte_carlo_ukf():
    ...
    plt.scatter(xs, ys, marker=".", alpha=0.2, color="k")
    plt.xlim(-25, 25)
    plt.ylim(-25, 25)
...
def Gaussian(mu, sigma, x):
    ...
    g = np.exp(-((mu - x) ** 2) / (sigma**2) / 2.0) / np.sqrt(2.0 * np.pi * (sigma**2))
...
def test_gaussian(N):
    for i in range(N):
        ...
        assert abs(d) < 1.0e-8, "{}, {}, {}, {}, {}, {}".format(
            d, mean, std, x, Gaussian(mean, std, x), scipy.stats.norm(mean, std).pdf(x)
        )
    ...
    for x in range(10):
        ...
        if x == 0 or x == 9:
            ...
            if x == 0:
                ...
                plt.scatter(
                    [x + 1], [x + 1], marker="x", color="r", s=180, label="True", lw=3
                )
